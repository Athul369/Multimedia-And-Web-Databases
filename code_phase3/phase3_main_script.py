...
from code.LocalBinaryPatterns import LBP
from code.ColorMoments import CM
from code.SIFT import SIFT
from code.HOGmain import HOG
from code.PrincipleComponentAnalysis import P_CA
from code.NonNegativeMatrix import NM_F
from code.SingularValueDecomposition import SVD
from code.LatentDirichletAllocation import LDA
from code.SimilarSubject import Subject
from code.Task8 import Task8
import code_phase3.PersonalizedPageRank as ppr
from code.feature_descriptor import *
...
if task_id == 0:
    ...
    """ Implement a program which, given a value k, creates an image-image similarity graph, such that from each
        image, there are k outgoing edges to k most similar/related images to it. Given 3 user specified imageids
        on the graph, the program identifies and visualizes K most dominant images using Personalized Page Rank (PPR)
        for a user supplied K. """
    if args.label == "None":
        print("Please provide Label")
        exit(1)
    """ Pass in the required inputs """
    ppr.identifyDominantImages(args.k_sim, args.k_dom, args.img_1, args.img_2, args.img_3)

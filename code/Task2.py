...
AllHands_metadata = imagedb["AllHands_metadata"]
...
def dorsal_palmar_cluster(label_path, unlabelled_path, model, k, c):
    ...
    palmar_img_list = []
    for descriptor in label_meta_obj.find():
        if descriptor["aspectOfHand"] == 'dorsal':
    ...
    for key, values  in results.items():
        ...
        for descriptor in AllHands_metadata.find():
            if descriptor["imageName"] == key:
                true_label = descriptor["aspectOfHand"]
...
def Query_input(c, folder, classfiy):
    ...
    dorsal_palmar_cluster(folder,classfiy ,"SIFT", 30, c)
    unlabelled_img_classification( classfiy, "SIFT")
